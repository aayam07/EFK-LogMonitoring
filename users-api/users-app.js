...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTH_SERVICE_SERVICE_HOST}/token/` + hashedPassword + '/' + password  // SERVICENAME_SERVICE_HOST is the internal automatic environment variable provided by kubernetes (which contains the IP address of the service) in the application running inside its cluster to access the pods inside the service internally
  );
